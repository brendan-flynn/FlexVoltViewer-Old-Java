...
//REG0
const char ChannelMask  = 0b11000000;
const char FreqIndexMask= 0b00111100;
const char DataModeMask = 0b00000010;
const char BitDepthMask = 0b00000001;
//REG1
const char FilterShiftValMask = 0b00011111;
...
void send8BitFilteredVals(void){
  ...
  if (downsampleCounter < DownSampleCount){
    for (int i = 0; i<NChan; i++){ // loop through channels
      val = analogRead(analogPins[i]);  // read channel.  10-bit ADC.  returns int
      val = val >> 2;
      filter_reg[i] = filter_reg[i] - (filter_reg[i] >> FilterShiftVal) + long(abs(val-128)+128);
    }
  }
